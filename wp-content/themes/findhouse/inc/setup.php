...
add_action('after_setup_theme', 'findhouse_setup');

if (!function_exists('findhouse_setup')) {
    /**
     ...
     */
    function findhouse_setup()
    {

        /*
         * Make theme available for translation.
         * Translations can be filed in the /languages/ directory.
         * If you're building a theme based on findhouse, use a find and replace
         * to change 'findhouse' to the name of your theme in all the template files
         */
        load_theme_textdomain('findhouse', get_template_directory() . '/languages');
        ...
        register_nav_menus(array(
            'primary' => esc_html__('Primary Menu', 'findhouse'),
        ));
        ...
        add_theme_support('custom-background', apply_filters('findhouse_custom_background_args', array(
        ...
        findhouse_setup_theme_default_settings();
    }
}
if (!function_exists('findhouse_import_attachment_image_size')) {
    /**
     ...
     *   );
     */
    function findhouse_import_attachment_image_size($url)
    {
...
add_action('wpopal_import_attachment_image_size', 'findhouse_import_attachment_image_size', 1, 2);

////////

add_filter('excerpt_more', 'findhouse_custom_excerpt_more');

if (!function_exists('findhouse_custom_excerpt_more')) {
    /**
     ...
     * @return string
     */
    function findhouse_custom_excerpt_more($more)
    {
        return '';
    }
}

add_filter('wp_trim_excerpt', 'findhouse_all_excerpts_get_more_link');

if (!function_exists('findhouse_all_excerpts_get_more_link')) {
    /**
     ...
     * @return string
     */
    function findhouse_all_excerpts_get_more_link($post_excerpt)
    {

        return $post_excerpt . ' <div class="findhouse-read-more-link-wrap"><a class="findhouse-read-more-link" href="' . esc_url(get_permalink(get_the_ID())) . '">' . esc_html__('Читать далее',
                'findhouse') . '</a></div>';
    }
}
add_filter('get_custom_logo', 'findhouse_custom_logo');

if (!function_exists('findhouse_custom_logo')) {
    function findhouse_custom_logo()
    {
 ...
 */
function findhouse_get_data_sample() {
    WP_Filesystem();
 ...
 */
function findhouse_import_sample_others() {

    $importer = new WpOpal_Core_Admin_Content_Importer();
    // for agent 
    $importer->set_data( findhouse_get_data_sample() );
...
add_action( "wpopal_import_sample_others", "findhouse_import_sample_others" );