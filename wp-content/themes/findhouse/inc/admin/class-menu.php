...
if (!class_exists("findhouse_Core_Admin_Menu")) {
    /**
     ...
     */
    class findhouse_Core_Admin_Menu
    {
        ...
        public function edit_tgmpa_notice_action_links($action_links)
        {
            ...
            if ('opal-theme-plugins' == $current_screen->id) {
                $link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="#opal-install-plugins">' . esc_attr__('Manage Plugins Below', 'findhouse') . '</a>';
            } else {
                $link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="' . esc_url(self_admin_url('admin.php?page=opal-theme-plugins')) . '#opal-install-plugins">' . esc_attr__('Go Manage Plugins', 'findhouse') . '</a>';
            }
        ...
        {
            wp_enqueue_style('findhouse-admin', get_stylesheet_directory_uri() . '/assets/css/admin.css', array());
        }
        ...
        public function get_admin_menu_args()
        {
            ...
            $menu_name = esc_html__('Wpopal', 'findhouse');

            return apply_filters('wpopal_admin_menu_args', array(
                'name' => $menu_name,
                'title' => esc_html__('Welcome', 'findhouse'),
        ...
        public function register_admin_menu()
        {
            ...
            add_theme_page(
                $menu_args['title'],
                esc_html__('ThemeLexus', 'findhouse'),
        ...
        {
            return esc_html__('Welcome to Use %s', 'findhouse');
        }
        ...
        protected function the_header($type = '')
        {
            ...
            <section class="jumbotron text-center p-4">
                ...
                <div class="container">
                    ...
                    <ul class="subtitle">
                        <li> <?php echo esc_html__('Version', 'findhouse'); ?>
                            : <?php echo esc_html($the_theme->get('Version')); ?> </li>
                        <li> <?php echo esc_html__('Information', 'findhouse'); ?>: <a class="theme-info-detail"
                                                                                             href="<?php echo esc_attr($the_theme->get('ThemeURI')); ?>"><?php esc_html_e('Click here', 'findhouse') ?></a>
                        </li>
                    </ul>
                    ...
                    <div class="row">
                        <div class="dashboard-intro wp-col-4">
                            <div class="theme-box-content">
                                <h3><?php esc_html_e('Import Demo', 'findhouse') ?></h3>
                                <?php if ($status['plugin_required']) { ?>
                                    <p class="text-description">
                                        <?php esc_html_e('Please check and make sure that all required plugins are set up on your website', 'findhouse') ?></p>
                                    <p>
                                        <a href="<?php echo esc_url(admin_url('admin.php?page=tgmpa-install-plugins')); ?>"
                                           class="button-second"><?php esc_html_e('Install Plugins', 'findhouse') ?></a>
                                    </p>
                                <?php } else { ?>
                                    <p class="text-description"><?php esc_html_e('Clone a demo site in few clicks', 'findhouse') ?></p>
                                    <a href="<?php echo esc_url(admin_url('admin.php?page=pt-one-click-demo-import')); ?>"
                                       class="button-second"><?php esc_html_e('Run Importer', 'findhouse') ?></a>
                                <?php } ?>
                            </div>
                        </div>
        ...
        public function system_check()
        {
            ...
            ?>
            <h2 class="medium-title pb-3 mt-lg-0"><?php esc_html_e('System Status', 'findhouse') ?></h2>
            <table class="table-check-status">
                ...
                <?php if ($status['memory_limit']) { ?>
                    <tr>
                        <td colspan="3" class="status-messenger">
                            <span class="status-rerequired"> <?php esc_html_e('Current memory limit is OK, however <u>256 MB</u> is recommended.', 'findhouse'); ?></span>
                        </td>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <td colspan="3" class="status-messenger">
                            <?php esc_html_e('Minimum <u>128 MB</u> is required, <u>256 MB</u> is recommended.', 'findhouse'); ?>
                        </td>
                    </tr>
                <?php } ?>
                <tr>
                    <td>
                        <?php esc_html_e(' PHP Time Limit', 'findhouse'); ?>
                    </td>
                    <td>
                ...
                <?php if ($data['time_limit'] < 60) { ?>
                    <tr>
                        <td colspan="3" class="status-messenger">
                            <span class="status-rerequired"> <?php esc_html_e('Minimum <u>60</u> is required, <u>180</u> is recommended.', 'findhouse'); ?></span>
                        </td>
                    </tr>
                <?php } elseif ($data['time_limit'] < 180) { ?>
                    <tr>
                        <td colspan="3" class="status-messenger">
                            <span class="status-rerequired"><?php esc_html_e('Current time limit is OK, however <u>180</u> is recommended.', 'findhouse'); ?> </span>
                        </td>
                    </tr>
                <?php } ?>
                <tr>
                    <td><?php esc_html_e('PHP Max Input Vars', 'findhouse'); ?></td>
                    <td>
                    ...
                    <div class="wp-col-4">
                        <div class="bg-white box-shadow theme-box-content">
                            ...
                            <img class="img-auto"
                                 ...
                                 title="wp opal">
                            <h3><?php esc_html_e('Support Cover', 'findhouse') ?></h3>
                            <p><?php esc_html_e('We give you a reply within 24 hours except during weekends.', 'findhouse') ?></p>
                            <div class="box-footer">
                                <a href="<?php echo esc_attr($project['support']); ?>" target="_blank"
                                   class="button button-primary"><?php esc_html_e('Submit Ticket', 'findhouse') ?></a>
                            </div>
                    ...
                    <div class="wp-col-4">
                        <div class="bg-white box-shadow theme-box-content">
                            ...
                            <img class="img-auto"
                                 ...
                                 title="wp opal">
                            <h3><?php esc_html_e('Documentation', 'findhouse') ?></h3>
                            <p><?php esc_html_e('Read all documentation for our themes. Any questions or training needed for WordPress themes can be  found here.', 'findhouse') ?></p>
                            <div class="box-footer">
                                <a href="<?php echo esc_attr($project['document']); ?>"
                                   class="button button-primary"><?php esc_html_e('Explore Now', 'findhouse') ?></a>
                            </div>
                        </div>
                    </div>
                    <div class="wp-col-4">
                        <div class="bg-white box-shadow theme-box-content box-info">
                            ...
                            <img class="img-auto"
                                 ...
                                 title="wp opal">
                            <h3><?php esc_html_e('Our Information', 'findhouse') ?></h3>

                            <p>
                                <?php esc_html_e('Checkout our site and social for more information.', 'findhouse') ?>
