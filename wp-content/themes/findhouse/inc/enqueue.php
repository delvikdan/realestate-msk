...
if ( ! function_exists( 'findhouse_theme_scripts_styles' ) ) {
	/**
	 * Load theme's JavaScript and CSS sources.
	 */
	function findhouse_theme_scripts_styles() {
		// Get the theme data.
		...
		wp_enqueue_style( 'findhouse-styles', get_stylesheet_directory_uri() . '/style'.$suffix.'.css', array(), $css_version );

//		$css_version = $theme_version . '.' . filemtime(get_template_directory() . '/style.css');
//		wp_enqueue_style( 'findhouse-styles', get_stylesheet_directory_uri() . '/style.css', array(), $css_version );
 
		/// 
		$js_version = $theme_version . '.' . filemtime(get_template_directory() . '/assets/js/theme'.$suffix.'.js');
		wp_enqueue_script( 'findhouse-scripts', get_template_directory_uri() . '/assets/js/theme'.$suffix.'.js', array('jquery'), $js_version, true );
		...
		if( !findhouse_is_enable_custom_fonts() ) {
			wp_enqueue_style( 'findhouse-fonts', findhouse_fonts_url(), array(), null );
		}

    }
}
add_action('wp_enqueue_scripts', 'findhouse_theme_scripts_styles', 100);


if( !function_exists("findhouse_3rd_scripts_styles") ){
	
	/**
	 * Load theme's JavaScript and CSS from 3 rd plugins.
	 */
	function findhouse_3rd_scripts_styles( ) {
		...
		// load script is page 
		//if( is_archive() && get_theme_mod('findhouse_blog_archive_layout') == 'masonry' ){
			wp_enqueue_script( 'imagesloaded' );
			wp_enqueue_script( 'jquery-isotope' );
		//}
		
		// check if enable elementor plugin to ovoid load duplicate css and script
		if( !findhouse_is_elementor_activated() ){
...
add_action('wp_enqueue_scripts', 'findhouse_3rd_scripts_styles', 1);


/**
 * Remove some duplicate lib styles
 */
function findhouse_dequence_styles() {
...
add_action('wp_print_styles', 'findhouse_dequence_styles', 1 );

/**
 * Register custom fonts.
 */
function findhouse_fonts_url() {
	$fonts_url = '';
     ...
     */
    $font = esc_html_x('on', 'Global font: on or off', 'findhouse');
