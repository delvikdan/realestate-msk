    ...
    var GoogleMapSearch = Opalestate.GooglemapSingle = function ( data ) {
        var initializePropertiesMap = function ( properties ) {
            ...
            for ( var i = 0; i < properties.length; i++ ) {
                ...
                var size = new google.maps.Size( 60, 60 );
                if ( window.devicePixelRatio > 1.5 ) {
                ...
                var pricet = properties[ i ].pricehtml;
                        
                if ( pricet === '0&#8381;' ) {
                  pricet = 'цена по запросу';
                }
               
                boxText.innerHTML = '<div class="media-top" style="background-image: url(\'' + properties[ i ].thumb + '\');"><a class="thumb-link" href="' + properties[ i ].url + '">' +
                    '</a></div>' +
                    '<div class="info-container media-body">' +
                    '<h5 class="prop-title"><a class="title-link" href="' + properties[ i ].url + '">' +
                    properties[ i ].title +
                    '</a></h5><p class="prop-address">' + properties[ i ].address +
                    '</p><p><span class="price text-primary">' + pricet + pricelabel +
