...
class Opalmembership_Gateway_Stripe extends Opalmembership_Abstract_Gateway {
        ...
        <div id="opalmembership-stripe-form">
            ...
            <p class="form-row validate-required">
                <label for="cc-exp" class="label-field">
					<?php esc_html_e( 'Expires (MM/YYYY)', 'opalmembership' ) ?>
                    <abbr class="required" title="required">*</abbr>
                </label>
	...
	public function validate( $fields = [] ) {
		...
		} elseif ( ! CreditCard::validDate( $year, $month ) ) {
			$errors[] = [
				'field'   => 'cc-exp',
		...
		} elseif ( ! $card['type'] || ! CreditCard::validCvc( $fields['cc-cvc'], $card['type'] ) ) {
			$errors[] = [
				'field'   => 'cc-cvc',
	...
	public function process( $payment_id, $posted ) {
		 ...
		 * Generate Token
		 */
		$tokens = $this->stripe_request( 'tokens', [
			'card' => [
	 ...
	 */
	protected function stripe_request( $api = 'charges', $params = [] ) {
		$response = wp_safe_remote_post( $this->api_uri . '/' . $api, [
			'method'     => 'POST',
			'headers'    => [
